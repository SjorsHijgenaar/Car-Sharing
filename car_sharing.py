...
def main():
  ...
  def find_best_package_combination(car: Car, distance, duration):
    ...
    while heap:
      ...
      minute_rate_costs = remaining_km * car.km_price + remaining_hrs * car.hourly_rate

      for _id, package, count in counts:
        # heuristic for finding and adding package
        if remaining_hrs == 0 and remaining_km == 0:
          continue
        # if 0 < minute_rate_costs < package.price:
        #   new_cost = cost + minute_rate_costs
        #   heapq.heappush(heap, (new_cost, (dis_dur[0] + remaining_km, dis_dur[1] + remaining_hrs), counts))
        if 0 < package.duration < duration or remaining_hrs == 0 or duration == remaining_hrs and package.duration > 0:        
  ...
  duration = 168
  for best in find_best_package_combination(sixt_bmw_i3, distance, duration):
    print(best) 
